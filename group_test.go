...
func TestGet(t *testing.T) {
	...
	for key, value := range db {
		// TODO: 本地读取
		if bytes, err := group.Get(key); err != nil || bytes.String() != value {
	...
	// TODO: 不存在的查询
	if bytes, err := group.Get("unknown"); err == nil {
		t.Fatalf("the value of `unknown` should be empty, but `%s` got", bytes)
	}
}
